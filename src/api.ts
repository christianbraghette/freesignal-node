...
import { KeyExchangeData, KeyExchangeDataBundle, LocalStorage, Crypto } from '@freesignal/interfaces';
import { Datastore } from './datastore';
import { Datagram, IdentityKeys, Protocols, UserId } from '@freesignal/protocol';
import { FREESIGNAL_MIME, FreeSignalAPI } from '@freesignal/protocol/api';
...
export class FreeSignalNode extends FreeSignalAPI {
    ...
    public constructor(secretSignKey: Uint8Array, secretBoxKey: Uint8Array, opts: {
        ...
        storage?: {
            ...
            datagrams?: LocalStorage<DatagramId, { datagram: Datagram, seen: boolean }>;
            inbox?: LocalStorage<UserId, DatagramId[]>;
            outbox?: LocalStorage<DatagramId, Datagram>;
            keystore?: LocalStorage<PublicKey, KeyExchangeDataBundle>;
        }
    } = { autostart: true }) {
        let { dbpath, autostart, storage } = opts;
        dbpath ??= './db';

        super({
            secretSignKey,
            secretBoxKey,
            sessions: storage?.sessions ?? new Datastore(`${dbpath}/sessions.db`),
            keyExchange: storage?.keyExchange ?? new Datastore(`${dbpath}/handshake.db`),
            users: storage?.users ?? new Datastore(`${dbpath}/users.db`)
        });

        this.dbPath = dbpath;
        this.datagrams = storage?.datagrams ?? new Datastore(`${dbpath}/messages.db`)
        this.inbox = storage?.inbox ?? new Datastore(`${dbpath}/inbox.db`);
        this.outbox = storage?.outbox ?? new Datastore(`${dbpath}/outbox.db`);
        this.keystore = storage?.keystore ?? new Datastore(`${dbpath}/keystore.db`);
